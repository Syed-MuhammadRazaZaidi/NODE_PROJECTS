...
let enemiesList: string[] = ["Skeleton", "Zombie", "Warrior", "Assassin"];
let maxEnemyHp: number = 75;
let enemyHitDamage: number = 25;

let playerHp: number = 100;
let playerHitDamage: number = 50;
let totalHealHp: number = 3;
let healHpChance: number = 50;     // Percentage
let healHpRefill: number = 30;
let playerLevel: number = 1;

let playing: boolean = true;

let getRandNo = (min: number, max: number): number => {
...
while (playing) {
    ...
    let randEnemy: string = enemiesList[Math.floor(Math.random() * enemiesList.length)];
    console.log(chalk.red(`${randEnemy}`), "Has Appeared To Fight\n");
    let enemyHp: number = getRandNo(1,maxEnemyHp);
    ...
    while (enemyHp > 0 && playerHp > 0) {
        ...
        switch (playerSelect.command) {
            ...
            case "Attack":
            let playerStrike: number = getRandNo(1, playerHitDamage);
            let enemyStrike: number = getRandNo(1, enemyHitDamage)
