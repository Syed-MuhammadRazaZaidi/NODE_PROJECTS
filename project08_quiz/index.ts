...
console.log(
    `                       ${chalk.bold.cyan("Welcome To TypeScript Quiz!")}`);
console.log(chalk.green("-".repeat(90)));
console.log(`                                ${chalk.green("Powered By")}`);
console.log(
    `                   ${chalk.bold.cyan(">>>>>")} ${chalk.bold.yellow("SYED MUHAMMAD RAZA ZAIDI")} ${chalk.bold.cyan("<<<<<\n")}`);
...
const quiz: {
...
} = await inquirer.prompt([
    {
        ...
        type: "list",
        message:
            "Q1) : What type of assignment is this variable, `const fullName: string = 'Dylan Israel';`?\n",
    ...
    },
    {
        ...
        type: "list",
        message:
            "Q2) : True or False: Interfaces are similar to type aliases, but only for object types?\n",
        choices: ["\tTrue", "\tFalse"],
    },
    {
        name: "question_3",
    ...
    },
    {
        ...
        type: "list",
        message:
            "Q4) :What is the type of the parameter: `function ex(param1?: string){}`?\n",
        choices: [
            "\tstring",
            "\tstring | null",
            "\tstring | undefined",
...
setTimeout(() => {
    ...
    console.log(
        chalk.bold.yellow(
            `                     >> Your TypeScript Quiz Score Is : ${score}/50! <<`
        )
    );
    console.log(chalk.yellow("-".repeat(90)));
}, 1250);
