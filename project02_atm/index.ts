...
async function atm () {
    ...
    let userName: {userId:string, userPassword:string, options:string[]} = await inquirer.prompt([    
    {
        type: "string",
        name: "userId",
    ...
    let randAmount: number = Math.round(Math.random()*100000);
...
if (userName.userId && userName.userPassword)
    ...
    if (userName.userId === "Raza" && userName.userPassword === "123") {
        ...
        while (loop) {
        let selectOptions: {options:string} = await inquirer.prompt([
            {
            type: "list",
            name: "options",
        ...
        let cashOptions: {cashWithdrawalOptions: string} = await inquirer.prompt([
            {
            type: "list",
        ...
        let fastCash: {fastCashOptions: number} = await inquirer.prompt([
            {
            type: "list",
        ...
        let enteredAmount:{userAmount:number} = await inquirer.prompt(
            {
            type: "number",
            name: "userAmount",
